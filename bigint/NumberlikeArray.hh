...
#define NUMBERLIKEARRAY_H
	...
	#ifdef __GNUC__
	  #define BIGINT_PUBLIC __attribute__ ((dllexport))
	#else
	  #define BIGINT_PUBLIC __declspec(dllexport) // Note: actually gcc seems to also supports this syntax.
	#endif
  #else
	#ifdef __GNUC__
	  #define BIGINT_PUBLIC __attribute__ ((dllimport))
	#else
	  #define BIGINT_PUBLIC __declspec(dllimport) // Note: actually gcc seems to also supports this syntax.
	#endif
  #endif
  #define BIGINT_LOCAL
#else
  #if __GNUC__ >= 4
	#define BIGINT_PUBLIC __attribute__ ((visibility ("default")))
	#define BIGINT_LOCAL  __attribute__ ((visibility ("hidden")))
  #else
	#define BIGINT_PUBLIC
	#define BIGINT_LOCAL
  #endif
#endif
