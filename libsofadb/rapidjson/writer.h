...
public:
	...
	Writer& BigInt(const Ch* digits, SizeType length)
	{
		Prefix(kNumberType);
		for (SizeType i = 0; i < length; i++)
			stream_.Put(digits[i]);
		return *this;
	}
...
protected:
	...
	void WriteDouble(double d) {
		char buffer[100];
#if _MSC_VER
		int ret = sprintf_s(buffer, sizeof(buffer), "%.20g", d);
#else
		int ret = snprintf(buffer, sizeof(buffer), "%.20g", d);
#endif
